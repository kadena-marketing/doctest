...
<head>
  <title>Pact REST API: Interact with Kadena Smart Contracts</title>
  <meta name="description" content="Explore the Pact REST API for seamless interaction with Kadena smart contracts. Learn about send, local, listen, poll, 
  SPV, and blockchain queries for efficient dApp development." />
</head>

# Pact REST API

The Pact REST API is a powerful interface for interacting with smart contracts on the Kadena blockchain. It provides developers with a set of endpoints to 
execute contract functions, query blockchain state, and manage transactions efficiently.
...
1. **Send**: Submit transactions to the blockchain for execution.
2. **Local**: Execute read-only queries or simulate transactions without affecting the blockchain state.
3. **Listen**: Set up listeners for specific blockchain events or transaction results.
4. **Poll**: Check the status of previously submitted transactions.
5. **SPV**: Verify transaction inclusion using Simple Payment Verification.
6. **Blockchain Queries**: Retrieve detailed information about blocks, transactions, and network state.
...
Remember, the Pact REST API is designed to be intuitive and developer-friendly. Don't hesitate to experiment with different endpoints to fully grasp their capabilities.